 ...
 */
package com.team319.robot.motion;
...
/**
 * OtfMotionProfile (On the Fly Motion Profile)
 *
 * Reads the {@link CombinedSrxMotionProfile} stored in the {@link TrajectoryManager}
 * and prepares it to be run on the SRXs
 *
 * @author mwtidd
 *
 */
public class OtfMotionProfile {
	...
	private List<CANTalon.TrajectoryPoint> trajectoryPoints;
	 ...
	 */
	public OtfMotionProfile(CANTalon talon, boolean isLeft) {
		_talon = talon;
		/*
	...
	private void startFilling() {
		...
		CombinedSrxMotionProfile latestProfile = TrajectoryManager.getInstance().getLatestProfile();

		if(isLeft){
			startFilling(latestProfile.getLeftProfile().getPoints(),
					latestProfile.getLeftProfile().getNumPoints());
		}else{
			startFilling(latestProfile.getRightProfile().getPoints(),
					latestProfile.getRightProfile().getNumPoints());
		}
	...
	private void startFilling(double[][] profile, int totalCnt) {
		...
		this.trajectoryPoints = new ArrayList<CANTalon.TrajectoryPoint>();
	...
	public CANTalon.TrajectoryPoint getActivePoint(){
		return _status.activePoint;
	}

	public double getElapsedTime(){
		return getActivePoint().timeDurMs * trajectoryPoints.indexOf(getActivePoint());
	}

	public double getTargetVelocity(){
		return getActivePoint().velocity;
	}

	public double getActualVelocity(){
		return _talon.getAnalogInVelocity();
	}

	public double getTargetDistance(){
		return getActivePoint().position;
	}

	public double getActualDistance(){
		return _talon.getAnalogInPosition();
	}
}
